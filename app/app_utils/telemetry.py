...
def setup_telemetry() -> str | None:
    ...
    if bucket and capture_content != "false":
        ...
        os.environ.setdefault(
            ...
            f"service.namespace=geap-agents,service.version={commit_sha}",
        )
        ...
        logging.info(
            ...
            "gs://your-bucket and OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT="
            "NO_CONTENT to enable)"
        )

    # Set up OpenTelemetry exporters for Cloud Trace and Cloud Logging.
    # Degrade gracefully when no Application Default Credentials are present
    # (e.g. local Docker runs without gcloud auth) rather than crashing the app.
    try:
        credentials, project_id = google.auth.default()
    except Exception as exc:  # DefaultCredentialsError and friends
        logging.warning(
            "No Application Default Credentials found — skipping cloud telemetry (%s)",
            exc,
        )
        return bucket
    if project_id is None:
        logging.warning(
            "google.auth.default() returned no project ID — skipping cloud telemetry"
        )
        return bucket
    try:
        ...
        logging.warning("Cloud telemetry setup failed — skipping (%s)", exc)
        return bucket
